...
class Translator {
  constructor(options = {}) {
    ...
    this._cache = new Map();
    this._currentLanguage = null; // Store the current language
    ...
    if (
      ...
      this._options.languages.includes(this._options.defaultLanguage)
    ) {
  ...
  _detectLanguage() {
    ...
    if (this._options.languages.includes(lang.substr(0, 2))) {
      const languageSwitcherImg = document.querySelector('#languageSwitcher img.flag-icon');
      languageSwitcherImg.src = "img/" + lang.substr(0, 2) + ".svg";
      languageSwitcherImg.alt = lang.substr(0, 2) + " Flag";
    }
  ...
  async load(lang) {
    if (!lang) {
      lang = this._detectLanguage();
    }
    if (!this._options.languages.includes(lang)) {
      return;
    }
    this._currentLanguage = lang; // Set the current language
    await this._translate(await this._getResource(lang));
  ...
  // New public method to get the currently set language
  getCurrentLanguage() {
    return this._currentLanguage || this._options.defaultLanguage;
  }
}
